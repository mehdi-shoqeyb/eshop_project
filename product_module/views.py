...
class ProductDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['banners'] = SiteBanner.objects.filter(is_active=True,position=SiteBanner.SiteBannerPosition.product_detail)
        ...
        context['related_products'] = group_list(list(Product.objects.filter(is_active=True,is_delete=False,brand_id=loaded_product.brand_id).exclude(pk=loaded_product.id).all()[:12]),3)
...
def product_categories_components(request: HttpResponse):
...
def product_brands_components(request: HttpResponse):
