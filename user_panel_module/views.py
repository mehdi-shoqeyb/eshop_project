    ...
    def post(self, request: HttpRequest):
        ...
        if form.is_valid():
            ...
            if current_user.check_password(form.cleaned_data.get('current_password')):
                if not current_user.check_password(form.cleaned_data.get('password')):
                    current_user.set_password(form.cleaned_data.get('password'))
                    current_user.save()
                    logout(request)
                    return redirect(reverse('login-page'))
                else:
                    form.add_error('password', 'کلمه عبور جدید و قبلی یکی می باشد')
            else:
                form.add_error('current_password', 'کلمه عبور قبلی اشتباه می باشد')
        context = {
            'form': form,
        }
        return render(request, 'user_panel_module/change_password_page.html', context)
